function [processedDataStruct, analysisFigure] = barNoiseAnalysis_OKR_Physiology...
    (data,recordingType, analysisType, epochsSelected, cellID)
...
% will throw an error
    ...
    if strcmp(recordingType, 'Extracellular')
        ...
        for i = 1:numel(epochsSelected)
            thisEpoch = data.(cellID).epochs(epochsSelected(i));
            ...
            spikeData = SpikeDetector_SK(stimResponse);
            ...
            if binary %for binary stimuli
                if ~check %for noncheckerboard stimuli
                    stimulus =(255.*2*meanIntensity.* (obj.noiseStream.rand(numBars,numFrames)>0.5));
                else %checkerboard stimuli
                    stimulus = (255.*2*meanIntensity.*(obj.noiseStream.rand(numBarsY, numBarsX, numFrames)>0.5));
                end
        ...
        hold on
        title(strcat(cellID, '- Bar Noise ', stimType, 'Receptive Field'));
